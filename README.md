...
### 🚫 管理保留名称文件(夹)
- 支持创建、删除和访问 Windows 系统保留的设备名称，如 `CON`, `PRN`, `AUX`, `NUL`, `COM1`-`COM9`, `LPT1`-`LPT9`。对于文件夹，您需要使用 `\\?\` 访问，例如 `\\?\C:\path\to\target`。

### 🔚 管理以点或空格结尾的文件(夹)
- 支持创建、删除和访问以点 (`.`) 或空格 (` `) 结尾的文件，例如 `...` 或 `My Folder `。对于文件, 您需要使用 `\\?\` 访问，例如 `\\?\C:\path\to\target`。

### 📏 长路径支持 (最长 32768 字符)
- 使用 `\\?\` 访问, 摆脱传统 Windows 文件路径 260 个字符的长度限制。
